...
uses
  ...
  ExtCtrls, DBCtrls, Buttons, jpeg, System.Actions, Vcl.Imaging.pngimage;
...
begin
  ...
  begin
    ...
    if Criptografa(EdtSenha.Text, 10) = DataUsuarios.DataSet.FieldByName
      ('SENHA').AsString then
      aVerifica := true;
  end;
  // verifica
  ...
  begin
    MessageDlg('Usuário ou senha inválidos. Verifique!!!', mtWarning,
      [mbOK], 0);
    if sContador > 2 then
    begin
      sOK := false;
  ...
  begin
    ...
    sNivelUsuario := DataUsuarios.DataSet.FieldByName('TIPO_USUARIO').AsInteger;
    Close;
  end;
