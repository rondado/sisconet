...
uses
  ...
  ACBrDFe, ACBrMail, frxClass, ACBrNFeDANFEFRDM, ACBrBlocoX, ACBrExtenso,
  ACBrBoleto, ACBrBoletoFCFortesFr, ACBrNFSe, ACBrCTe, ACBrNCMs,
  ACBrDANFCeFortesFr, ACBrSATExtratoESCPOS, ACBrPosPrinter, ACBrSATExtratoClass,
  ACBrSATExtratoReportClass, ACBrSATExtratoFortesFr, ACBrSAT, ACBrNFeDANFeESCPOS,
  ACBrDFeDANFeReport, ACBrDFeReport;
...
type
  TDmAcBr = class(TDataModule)
    ...
    ACBrValidador_SSL: TACBrValidador;
    ACBrNFe1: TACBrNFe;
    ACBrNFe2: TACBrNFe;
    ...
    procedure ACBrECF1DepoisFechaCupom(const Observacao: String;
      const IndiceBMP: Integer);
    procedure ACBrECF1DepoisFechaNaoFiscal(const Observacao: String;
      const IndiceBMP: Integer);
    procedure ACBrECF1DepoisLeituraX(Sender: TObject);
    procedure ACBrECF1DepoisReducaoZ(Sender: TObject);
    procedure ACBrECF1DepoisSangria(const Valor: Double; const Obs,
      DescricaoCNF, DescricaoFPG: String);
    procedure ACBrECF1DepoisSuprimento(const Valor: Double; const Obs,
      DescricaoCNF, DescricaoFPG: String);
    procedure ACBrECF1ErrorSemPapel(Sender: TObject);
    procedure ACBrECF1DepoisAbreCupom(const CPF_CNPJ, Nome,
      Endereco: String);
...
const
  ChavePrivada = '-----BEGIN RSA PRIVATE KEY-----'+#13+#10+
                 'MIICWwIBAAKBgQDv2KaFWYuSl1XaCY1JO0bm7zsPcnrMHZUQoZ+ZJnVAUPXjUA6W'+#13+#10+
                 'Rd927j6lv/gUoiCzfr3/e/7XMfjWCZWzY0k4Nu9joJmDKV2J0Bac/2lx4yw1kPTx'+#13+#10+
                 'cQN7SEp5rpZqU+2Hyx3HfSR9lLcI5rLE/8nCJ50a2qEzt6t9S+J/rA9AZwIDAQAB'+#13+#10+
                 'AoGAE1lP/bv+lYHd7CwPGNROYPz1r8ZVf7f3OZNAU9OVa+3cGE1yD89tOqP3q9lz'+#13+#10+
                 'wv5dUv2riDvRkDy4zVOSnxkoY9eVpWYu5mGjKYw4vxGNgLSI4+HVfteVUTCM6xhd'+#13+#10+
                 'bxO6wt9fPoGMPAX40f70JgXfeiV5Y758gaYBygBhC4XnEGECQQD83C+LZzUbR8Ow'+#13+#10+
                 '5MVdqS7bA4OoxBsfvrYf2U87kS6k+h/xI4kKt9f3H5rPYakFir4cDR1vTo4SBniU'+#13+#10+
                 'fy1bV4jxAkEA8tMYZPqwzqCvDhaE3dA6aUMQY5jDel68D7GhgIESX2dLqgkRCdkt'+#13+#10+
                 'quouuCnK9/6din7Wo+oZr75LKQdsg28e1wJALH1lVCbNnkYrNVWbMFaTKu/cW3Zd'+#13+#10+
                 'u2En1ES8/yMVTOvBcQEqnJsqQY3rBDAPuCzoNDFOi4YL0zoWPzJ9PYQSUQJASiqB'+#13+#10+
                 'q+4J7MKVVTc5k8pS3O235sO1CP5AcIavrmE97FfPe3rTcm+cDDUE2gkhzkc9dRyy'+#13+#10+
                 'kFQjTlNP/5Md6LMa9wJAVrY3epef/RnceFzp4nro90ttS+eZKHDrnTuYGhCWijpB'+#13+#10+
                 'NiE7ZOhFKU3EdAQgj4hacsfAe93g/epUmQaThz4FxA=='+#13+#10+
...
var sEstAnterior, sEstAtual: String;
begin
//  sMensagem.Panels[1].Text := Format('Anterior: %s - Atual: %s', [sEstAnterior, sEstAtual]);
end;

procedure TDmAcBr.ACBrECF1DepoisFechaCupom(const Observacao: String;
  const IndiceBMP: Integer);
begin
  xDataECF :=  StrToDateTime(FormatDateTime('dd/mm/yyyy',ACBrECF1.DataHora));
  xHoraECF := StrToDateTime(FormatDateTime('hh:mm:ss',ACBrECF1.DataHora));
...
procedure TDmAcBr.ACBrECF1DepoisFechaNaoFiscal(const Observacao: String;
  const IndiceBMP: Integer);
begin
  xDataECF :=  StrToDateTime(FormatDateTime('dd/mm/yyyy',ACBrECF1.DataHora));
  xHoraECF := StrToDateTime(FormatDateTime('hh:mm:ss',ACBrECF1.DataHora));
...
procedure TDmAcBr.ACBrECF1DepoisSangria(const Valor: Double; const Obs,
  DescricaoCNF, DescricaoFPG: String);
begin
  xDataECF :=  StrToDateTime(FormatDateTime('dd/mm/yyyy',ACBrECF1.DataHora));
  xHoraECF := StrToDateTime(FormatDateTime('hh:mm:ss',ACBrECF1.DataHora));
...
procedure TDmAcBr.ACBrECF1DepoisSuprimento(const Valor: Double; const Obs,
  DescricaoCNF, DescricaoFPG: String);
begin
  xDataECF := StrToDateTime(FormatDateTime('dd/mm/yyyy',ACBrECF1.DataHora));
  xHoraECF := StrToDateTime(FormatDateTime('hh:mm:ss',ACBrECF1.DataHora));
...
procedure TDmAcBr.ACBrECF1DepoisAbreCupom(const CPF_CNPJ, Nome,
  Endereco: String);
begin
  xDataECF := StrToDateTime(FormatDateTime('dd/mm/yyyy',ACBrECF1.DataHora));
  xHoraECF := StrToDateTime(FormatDateTime('hh:mm:ss',ACBrECF1.DataHora));
...
procedure TDmAcBr.ACBrECF1DepoisAbreRelatorioGerencial(
