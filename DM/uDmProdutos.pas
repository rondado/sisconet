//fabricio novo incluido a tabela empenho
unit uDmProdutos;
...
uses
  ...
  Datasnap.Provider, Data.DB, Data.SqlExpr, Windows, Messages, Variants,
  Graphics, Controls, Forms, ExtCtrls, ComCtrls, ImgList, Dialogs, Menus,
  uDmAcBr, ACBrDevice, pcnConversao, IniFiles, ACBrPAFClass, StdCtrls, ActnList,
  jpeg, OleCtrls, SHDocVw, unitVidaLink, CharPrinter, ACBrSintegra,
  pcnConversaoNFe, ACBrUtil, pcnRede, ACBrSATClass, ACBrPosPrinter, ACBrBlocoX,
  ACBrBlocoX_Comum;

type
  TDmProdutos = class(TDataModule)
    QryFechamentoEst: TSQLQuery;
    ...
    QryProdutoCor: TSQLQuery;
    QryProdutoTamanho: TSQLQuery;
    ...
    QryProduto_AliquotaIMPOSTO_PFCP: TFloatField;
    QryProduto_AliquotaPFCP_EMBUTIDO: TStringField;
    CdsProduto_AliquotaIMPOSTO_PFCP: TFloatField;
    CdsProduto_AliquotaPFCP_EMBUTIDO: TStringField;
    QryProdutosFRETE: TFloatField;
    QryProdutosCUSTO_OPER: TFloatField;
    QryProdutosOUTRAS_DESP: TFloatField;
    QryProdutosALIQ_SSIMPLES: TFloatField;
    QryProdutosDESCONTO: TStringField;
    CdsProdutosFRETE: TFloatField;
    CdsProdutosCUSTO_OPER: TFloatField;
    CdsProdutosOUTRAS_DESP: TFloatField;
    CdsProdutosALIQ_SSIMPLES: TFloatField;
    CdsProdutosDESCONTO: TStringField;
    ...
    QryNCMVIGENCIAINICIO: TDateField;
    QryNCMVIGENCIAFIM: TDateField;
    ...
    QryProdutosA_GRANEL: TStringField;
    QryProdutosITEM_A_COMPOR: TStringField;
    CdsProdutosA_GRANEL: TStringField;
    CdsProdutosITEM_A_COMPOR: TStringField;
    ...
    procedure DspProdutoCompostoGetTableName(Sender: TObject; DataSet: TDataSet;
      var TableName: string);
    ...
    procedure DspProdutoAuxGetTableName(Sender: TObject; DataSet: TDataSet;
      var TableName: string);
    ...
    procedure DspTabelaGetTableName(Sender: TObject; DataSet: TDataSet;
      var TableName: string);
    procedure DspProdutoFornecedorGetTableName(Sender: TObject;
      DataSet: TDataSet; var TableName: string);
    procedure DspProduto_AliquotaGetTableName(Sender: TObject;
      DataSet: TDataSet; var TableName: string);
    procedure CdsProduto_AliquotaNewRecord(DataSet: TDataSet);
    procedure CdsMovimentoEstNewRecord(DataSet: TDataSet);
    procedure DspMovimentoEstGetTableName(Sender: TObject; DataSet: TDataSet;
      var TableName: string);
    procedure DspFechamentoEstGetTableName(Sender: TObject; DataSet: TDataSet;
      var TableName: string);
    procedure CdsFechamentoEstNewRecord(DataSet: TDataSet);
    procedure DspSubGrupoGetTableName(Sender: TObject; DataSet: TDataSet;
      var TableName: string);
    procedure DspGrupoGetTableName(Sender: TObject; DataSet: TDataSet;
      var TableName: string);
    procedure DspNCMGetTableName(Sender: TObject; DataSet: TDataSet;
      var TableName: string);
    procedure DspCestGetTableName(Sender: TObject; DataSet: TDataSet;
      var TableName: string);
    procedure CdsNCMNewRecord(DataSet: TDataSet);
    procedure DspProdutoCorGetTableName(Sender: TObject; DataSet: TDataSet;
      var TableName: string);
    procedure DspProdutoTamanhoGetTableName(Sender: TObject; DataSet: TDataSet;
      var TableName: string);
    procedure CdsProdutoCorNewRecord(DataSet: TDataSet);
    procedure CdsProdutoTamanhoNewRecord(DataSet: TDataSet);
    procedure DataModuleCreate(Sender: TObject);
    procedure CdsEmpenhoNewRecord(DataSet: TDataSet);
  private
    { Private declarations }
    procedure FormataCampos;
    function InsereItemCupom(pQuant, pAcres, pDesc: Double;
      pPercentual: Boolean): boolean;
    procedure InsereRegistroProduto(id, grupo, subgrupo: Integer; descricao,
      referencia, ean13, unidade, codigo_ncm, ippt, iat, cst_interno,
      cfop_interno, csosn, bens_consumo, cst_pis, cst_cofins: String; quant_caixa,
      aliquota_icms, aliquota_ipi, estoque, preco_compra, preco_venda, perc_com,
      margem_lucro, mva, aliquota_pis, aliquota_cofins: Double);
  public
    { Public declarations }
    ...
    function Dados_Produto2(pDatai:TDateTime=0;pDataf:TDateTime=0;
...
uses uDmConexao, uLibrary, uDmWorkCom, uDmAuxiliar;

{$R *.dfm}

procedure TDmProdutos.CdsEmpenhoNewRecord(DataSet: TDataSet);
begin
  DataSet.FieldByName('ID').AsInteger        := GenID('EMPENHO');
end;
...
begin
  ...
  begin
    ...
    begin
      MessageDlg('SoftWare de Demonstração!'+#13+#10+
                 'Não é permitido ultrapassar o limite de produtos estabelecido,'+#13+#10+
                 'Máximo '+IntToStr(pNumProd)+'. Entre em contato com o suporte técnico!!!', mtWarning, [mbOK], 0);
      Abort;
    end;
  end;
  ...
  DataSet.FieldByName('COMBUSTIVEL').AsString       := 'N';
    ...
    //rondado alteração A_Granel
  DataSet.FieldByName('A_GRANEL').AsString       := 'N';
  DataSet.FieldByName('ITEM_A_COMPOR').AsString       := 'N';
...
function TDmProdutos.Dados_Produto2(pDatai, pDataf: TDateTime;
  ApenasProdMovto: Boolean): Boolean;
...
function TDmProdutos.Dados_Produto_Aliquota(pIDProd: Integer;
  pUF: String): Boolean;
begin
...
procedure TDmProdutos.DspCestGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: string);
begin
  TableName := 'TAB_CEST';
...
procedure TDmProdutos.DspEstoqueGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: string);
begin
  TableName := 'ESTOQUE';
...
procedure TDmProdutos.DspFechamentoEstGetTableName(Sender: TObject;
  DataSet: TDataSet; var TableName: string);
begin
...
procedure TDmProdutos.DspGrupoGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: string);
begin
  TableName := 'GRUPO';
...
procedure TDmProdutos.DspMovimentoEstGetTableName(Sender: TObject;
  DataSet: TDataSet; var TableName: string);
begin
...
procedure TDmProdutos.DspNCMGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: string);
begin
  TableName := 'NCM';
...
procedure TDmProdutos.DspProdutoAuxGetTableName(Sender: TObject;
  DataSet: TDataSet; var TableName: string);
begin
  TableName := 'PRODUTOS';
...
procedure TDmProdutos.DspProdutoCompostoGetTableName(Sender: TObject;
  DataSet: TDataSet; var TableName: string);
begin
...
procedure TDmProdutos.DspProdutoCorGetTableName(Sender: TObject;
  DataSet: TDataSet; var TableName: string);
begin
...
procedure TDmProdutos.DspProdutoFornecedorGetTableName(Sender: TObject;
  DataSet: TDataSet; var TableName: string);
begin
...
procedure TDmProdutos.DspProdutosGetTableName(Sender: TObject;
  DataSet: TDataSet; var TableName: string);
begin
  TableName := 'PRODUTOS';
...
procedure TDmProdutos.DspProdutoTamanhoGetTableName(Sender: TObject;
  DataSet: TDataSet; var TableName: string);
begin
  TableName := 'PRODUTO_TAMANHO';
end;

procedure TDmProdutos.DspProduto_AliquotaGetTableName(Sender: TObject;
  DataSet: TDataSet; var TableName: string);
begin
...
procedure TDmProdutos.DspSubGrupoGetTableName(Sender: TObject;
  DataSet: TDataSet; var TableName: string);
begin
  TableName := 'SUBGRUPO';
...
procedure TDmProdutos.DspTabelaGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: string);
begin
  TableName := 'PRODUTOS';
...
begin
  ...
  CdsTabelaPRECO_VENDA.DisplayFormat  := DmWorkCom.PAFECF.gMascaraDec;
    //fabricio: linha 1227 a 1236 - segue abaixo
  CdsProdutosMARGEM_LUCRO.DisplayFormat  := '##,##0.00';
  CdsProdutosMARGEM_LUCRO.EditFormat     := '##,##0.00';
  CdsProdutosFRETE.DisplayFormat  := '##,##0.00';
  CdsProdutosFRETE.EditFormat     := '##,##0.00';
  CdsProdutosOUTRAS_DESP.DisplayFormat  := '##,##0.00';
  CdsProdutosOUTRAS_DESP.EditFormat     := '##,##0.00';
  CdsProdutosCUSTO_OPER.DisplayFormat  := '##,##0.00';
  CdsProdutosCUSTO_OPER.EditFormat     := '##,##0.00';
  CdsProdutosALIQ_SSIMPLES.DisplayFormat  := '##,##0.00';
  CdsProdutosALIQ_SSIMPLES.EditFormat     := '##,##0.00';
...
procedure TDmProdutos.GravaFechamentoProd(fIDProd: Integer; fMes, fAno: String;
  fQuant: Double);
begin
  Dados_FechamentoEst(0);
  try
...
procedure TDmProdutos.Grava_TabMovEst(fTipoMov, fIdProduto: Integer;
  fQuant: Double);
begin
  // fTipoMov = 0 - Baixar
  //            1 - Adcionar
...
function TDmProdutos.InsereItemCupom(pQuant, pAcres, pDesc: Double;
  pPercentual: Boolean): boolean;
begin

end;

procedure TDmProdutos.InsereRegistroProduto(id, grupo, subgrupo: Integer;
  descricao, referencia, ean13, unidade, codigo_ncm, ippt, iat, cst_interno,
  cfop_interno, csosn, bens_consumo, cst_pis, cst_cofins: String; quant_caixa,
  aliquota_icms, aliquota_ipi, estoque, preco_compra, preco_venda, perc_com,
  margem_lucro, mva, aliquota_pis, aliquota_cofins: Double);
var SQLPaf : TSQLQuery;
begin
  try
    SQLPaf := TSQLQuery.Create(Application);
    SQLPaf.SQLConnection := DmConexao.ConexaoDBX;
    SQLPaf.Close;
    SQLPaf.SQL.Clear;
    ...
    SQLPaf.Params.ParamByName('csosn_interno').AsString := csosn;
...
var SQLRet : TSQLQuery;
    aBensConsumo : String;
    aIDProduto, aCodGrupo, aCodSubGrupo : Integer;
    aCFOP, aCSOSN, aNCM : String;
begin
  ...
  try
    ...
    //
    SQLRet := TSQLQuery.Create(Application);
    with SQLRet do
    begin
      ...
      SQL.Add('       p.margem_agregada,p.classificacao_fiscal,p.csosn');
      ...
      begin
        ...
        if not FieldByName('codsubgrupo').IsNull then
           ...
           aCodSubGrupo := 1;
        //
        aCFOP := Copy(Trim(FiltraNumero(FieldByName('codaliquota').AsString)),1,4);
        if Trim(aCFOP) = '' then
           aCFOP := '5102';
        aCSOSN := Copy(Trim(FiltraNumero(FieldByName('csosn').AsString)),1,3);
        if Trim(aCSOSN) = '' then
           aCSOSN := '102';
        // captura o NCM
        aNCM := FiltraNumero(Copy(FieldByName('ncm').AsString,1,8));
        if Trim(aNCM) = '' then
           aNCM := FiltraNumero(Copy(FieldByName('classificacao_fiscal').AsString,1,8));
        //
        aBensConsumo := 'N';
        ...
        GeraTributacaoProduto(aIDProduto,
                              DmWorkCom.CdsEmpresaUF.AsString,
                              FieldByName('cst').AsString,
                              aCFOP,
                              aCSOSN,
                              FieldByName('aliquota').AsFloat,
                              0);
        //
        Next;
      end;
