...
procedure TFrmContas.PassaPeriodoRelatorio(pDatai,pDataf:TdateTime;totalconta,totalquitado,totalnegociado:currency);
begin
  with CdsPeriodo do
  begin
    if not Active then
...
procedure TFrmContas.CdsClienteAfterOpen(DataSet: TDataSet);
begin
  inherited;
  LbClientes.Caption:= FormatFloat('000',CdsCliente.RecordCount);
end;

procedure TFrmContas.CdsConsultaAfterOpen(DataSet: TDataSet);
var
...
procedure TFrmContas.DBGrid1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
valor_conta, valor_quitado, valor_negociado: currency;
...
procedure TFrmContas.EdtLocalizarKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  TabEnter(Self,Key);
end;
