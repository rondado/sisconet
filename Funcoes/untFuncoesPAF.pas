...
var
  {variavel para controle de exibeção de mensagens}
  Hwnd : THandle;
...
function stEncrypt(S : string) : string;
// Função auxiliar para criptografar
begin
  Result := Encrypt(S, 1024);
end;

function stDecrypt(S : string) : string;
// Função auxiliar para descriptograr
begin
...
function ConectarBanco(SQLConnection : TSQLConnection; caminho : string; banco : string) : TStringList;
//**********************************************************************************************************************************
// Esta função tem por objetivo conectar a um banco de dados firebird
// Ela recebe como parâmetro o sqlconection, o caminho do banco, o usuario e a senha
...
begin
  ...
  if controle > 0 then
     begin
       msg := 'Arquivos de configuração inexistentes.' + chr(10) + 'Comunique imediatamente a RoraimaSoft.';
     ...
     begin
        ...
        while not Eof (arq) do
          begin
            ...
            ReadLn(arq,linha02);          // sem utilização por enquanto
            ReadLn(arq,usuario);          // LI O USUÁRIO
...
procedure Mensagem(msg : string; tipo : string);
//******************************************************************************
// Esta função mostra a mensagem usando o Application.Messsagebox
// Desenvolvida por: Paulo Rodrigues
var
  usuario : string;
begin
...
begin
  with DataSetLocal do
    begin
      ...
      Open;
      if (FieldByName('TURNO').AsString) = '' then // não existe caixa aberto
        begin
          Result := false;
        end
      else
...
begin
  ...
  try
    ...
    // Se aplicação estiver em uso executa
    if Hwnd <> 0 then
    begin
