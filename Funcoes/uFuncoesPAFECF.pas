...
uses
  ...
  ACBrDevice, ACBrBAL, DBClient, uMensagem, CharPrinter, Printers, WinSpool;
...
type
   ...
   Public
    ...
    // Balanca
    gModeloBal, gHandShake, gParity, gStopBits,
    gDataBits, gDecimais, gAbastCupom, gNumDigito : Integer;
    ...
    gPathTEF : String;
    // Dados da Redução
    gVendaBruta, gGrandeTotal, gCancelados, gDescontos,
    gSubstituicao, gNaoTributado, gIsentos : Double;
    ...
    pModeloECF, gBinCartao, gImpressora: String;
    pSerial,pVersao,pDataSP,pHoraSP : String;
    gValorSangria : Double;
    gIndiceGerencial, gIndiceIdentifica, gIndiceParametro, gIndiceMesaAbr,
    gIndiceConfMesa, gIndiceTransfMesa, gIndiceAfericao, gIndiceEstFisico,
    gIndicePerdaCom, gIndiceRetCom, gIndiceSemCom, gIndicePerdaEst,
    gIndiceDescontinua : Integer;
    ...
    function AnalizaArquivoRetorno(var Aprovado:Boolean;var Mensagem:String;var EhCheque:Boolean;pControle:Integer;ArqOrigCancelamento:String=''):Boolean;
    ...
    procedure MemoriaFiscal(pInicio,pFim:String;
                            pDatai,pDataf:TDateTime;
                            pTipo,pInterval,pTipoLeitura:Boolean;
                            pNomeArq:String);
    ...
    procedure VerificaResolvePendenciaTEF(CdsPedidoFP:TClientDataSet;
                                          FechamentoCupom:Boolean=false;
                                          AberturaSistema:Boolean=false);
    ...
    procedure EmiteGerencialSangriaNFCe(pCaixa,pOperador:String;pData:TDateTime;pValor:Double;pMotivoRetirada:String);
    ...
    procedure ImprimirFechamentoCaixaNFCe(Operador,Caixa:String;Valor:Double);
    ...
    procedure RelatorioDescontinuidadeEncerrante(Bomba,Bico,Motivo,Justifica,
                                                 Justifica1,Justifica2,Medidas,
                                                 Medidas1,Medidas2:String;
                                                 Data,hora:TDateTime;
                                                 EI,EF:Double);
...
Uses  Libeay32, uDmAcBr, ACBrECF, uDmPosto, uDmWorkCom; //fabricio acrescido o udmworkcom.
...
var cLinhaArquivo, cConteudo,cCampoArquivo,aArquivo: string;
    cArquivo: TextFile;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (copy(cLinhaArquivo,1,3) = '002') or (copy(cLinhaArquivo,1,3) = '010') or
         (copy(cLinhaArquivo,1,3) = '012') or (copy(cLinhaArquivo,1,3) = '027') then
    ...
    cConteudo   := '000-000 = CNF' + #13 + #10+
                   '001-000 = ' + FormatDateTime('hhmmss',Time) + #13 + #10 + cConteudo;
    Sleep(1000);
...
function TFuncoesPAF.NaoConfirmaTransacao(pControle:Integer;pArquivo:String;
  pExibeMensagemTef:Boolean=True): boolean;
var cLinhaArquivo, cConteudo, cCampoArquivo, AuxMensagem, aArquivo: string;
    cArquivo: TextFile;
begin
  ...
  begin
    ...
    cConteudo := '000-000 = NCN' + #13 + #10+
                 '001-000 = ' + FormatDateTime('hhmmss',Time) + #13 + #10;
    Reset(cArquivo);
    while not System.EOF(cArquivo) do
    begin
       ReadLn(cArquivo,cLinhaArquivo);
...
//          1: cConteudo := cConteudo + cLinhaArquivo + #13 + #10;
          3: gValor := copy( cLinhaArquivo, 11, Length( cLinhaArquivo ) - 10 );
          10: begin
             cConteudo := cConteudo + cLinhaArquivo + #13 + #10;
    ...
    if sTipoTEF in [0,2] then
    begin
      AuxMensagem := 'Cancelada a Transação' + #13 + #13+
                     'Rede:   ' + gNomeRede + #13;
      if gNSU <> '' then
...
begin
  ...
  begin
    ...
    begin
       ...
       cCampoArquivo := copy(cLinhaArquivo,1,3);
       case StrToInt(cCampoArquivo) of
         10: cConteudo := copy(cLinhaArquivo,11,Length(cLinhaArquivo)-10);
       end;
    end;
    CloseFile(cArquivo);
    // verifica a bandeira
...
begin
   ...
   begin
     cConteudoArquivo := '000-000 = ADM'+#13+#10+
                         '001-000 = ' +gIdentifica+#13+#10+
                         '701-000 = ' + gNOME_EXECUTAVEL+' v'+gVERSAO  + #13 + #10 +
                         '706-000 = ' + '3' + #13 + #10 +
                         '716-000 = ' + gRazao_social + #13 + #10 +
                         '999-999 = 0';
   end
   else
   begin
  ...
  // Adiciona para atender CIELO PP
                         '210-084 = ' + 'RRSOFT1010' + #13 + #10 +
                         '210-084 = ' + Copy(gNOME_EXECUTAVEL,1,6)+Copy(FiltraNumero(gVERSAO),1,2)+'10' + #13 + #10 +
                         '701-034 = ' + '4' + #13 + #10 +
  //
                         '999-999 = 0';
   end;
...
var Aprovado, EhCheque, Impresso : Boolean;
...
//          1: cConteudo := cConteudo + cLinhaArquivo + #13 + #10;
          ...
          22:  cConteudo := cConteudo + cLinhaArquivo + #13 + #10;
          23:  cConteudo := cConteudo + cLinhaArquivo + #13 + #10;
          27:  cConteudo := cConteudo + cLinhaArquivo + #13 + #10;
          999: cConteudo := cConteudo + cLinhaArquivo;
       end;
    end;
   ...
   begin
     ...
     begin
       cConteudoArquivo := '000-000 = CHQ' + #13 + #10+
                           '001-000 = ' + gIdentifica  + #13 + #10 +
                           '002-000 = ' + gNumeroCupom + #13 + #10 +
                           '003-000 = ' + gValor + #13 + #10 +
                           '004-000 = ' + '0' + #13 + #10 +
                           '701-000 = ' + gNOME_APLICACAO+' v'+gVERSAO  + #13 + #10 +
                           '706-000 = ' + '3' + #13 + #10 +
                           '716-000 = ' + gRazao_social + #13 + #10 +
                           '999-999 = 0';
     end
     ...
     begin
       cConteudoArquivo := '000-000 = CHQ' + #13 + #10+
                           ...
                           '004-000 = ' + 'R$' + #13 + #10 +
    // Adiciona para atender CIELO PP
                           '210-084 = ' + copy(gNOME_APLICACAO,1,6)+Copy(FiltraNumero(gVERSAO),1,2)+'10' + #13 + #10 +
                           '701-034 = ' + '4' + #13 + #10 +
                           '999-999 = 0';
     end;
   end
...
function TFuncoesPAF.AnalizaArquivoRetorno(var Aprovado:Boolean;var
  Mensagem:String; var EhCheque:Boolean;pControle:Integer;ArqOrigCancelamento:String=''):Boolean;
var AuxTXT, cLinhaArquivo, cCampoArquivo: string;
    cArquivo  : TextFile;
...
//                      gPossuiCampo711 := true;
  ...
  else MessageDlg('Erro ao comunicar com o Gerenciador Padão!', mtWarning, [mbOK], 0);
  CloseFile( cArquivo );
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        try
          formMensagem := TFormMensagem.Create('Aguarde... GT Invalido. Conferindo dados do sistema!',550,150,clBlack,clSilver);
        ...
        begin
          if (aCRZ_Banco = aCRZ_ECF) and
             (aVB_Banco = aVB_ECF) then
          begin
            aRecupera := true;
          end;
        end;
...
begin
  ...
  try
    ...
    begin
      DmAcBr.ACBrECF1.AbreCupom(ClienteCupom.CPF,
                                ClienteCupom.Cliente,
                                ClienteCupom.Endereco+','+
                                ClienteCupom.Numero+'-'+
                                ClienteCupom.Bairro);
    end
    else
...
function TFuncoesPAF.ImprimeItemECF(pUnid,pEAN,pDesc,pAliq,pTipo,
  tipoDescAcre:String;pQuant,pVal,pDesconto:Double): Boolean;
begin
  Result := true;
  try
    DmAcBr.ACBrECF1.VendeItem(pEAN,
                              pDesc,
                              pAliq,
                              pQuant,
                              pVal,
...
function TFuncoesPAF.AbreCupomNaoFiscalECF(pTipoRel:Integer;pFormaPag,
  pCOO:String;pValor:Double): Boolean;
begin
  Result := true;
  if pTipoRel = 0 then
...
procedure TFuncoesPAF.MemoriaFiscal(pInicio,pFim:String;pDatai,pDataf:TDateTime;
  pTipo,pInterval, pTipoLeitura:Boolean;pNomeArq:String);
...
function TFuncoesPAF.EspelhoMFD(pArquivo,pInicio,pFim:String;pDatai,
  pDataf:TDateTime;pinterval: Boolean):Boolean;
var Datai, Dataf : TDateTime;
begin
  // pinterval - true  = por data
  //             false = por redução
...
begin
  Result := true;
  try
//    DmAcBr.ACBrECF1.PafMF_ArqMF(pArquivo);
...
begin
  Result := true;
  try
//    DmAcBr.ACBrECF1.PafMF_ArqMFD(pArquivo);
...
var hora,minuto,segundo,miliseg : word;
begin
  Result := true;
  try
  ...
  try
    ...
    try
      ...
      begin
        ...
        begin
          if (F.Name <> 'ConfigPDV.exe') and
             (F.Name <> 'Gestor.exe') and
             (F.Name <> 'Sintegra.exe') and
             (F.Name <> 'Relatorios.exe') and
             (F.Name <> 'GeradorSPED.exe') and
             (F.Name <> 'NFCe.exe') and
             (F.Name <> 'SPED.exe') and
             (F.Name <> '_QrCode.dll') or
             (F.Name <> 'Atualiza_MD5.exe') then
          begin
    ...
    begin
      ...
      case StrToInt( cCampoArquivo ) of
        9:begin
            ...
            if ( aRetTransacao ) = 'FF' then aux := 1;
          end;
      end;
    end;
    CloseFile( cArquivo );
...
procedure TFuncoesPAF.VerificaResolvePendenciaTEF(CdsPedidoFP:TClientDataSet;
  FechamentoCupom:Boolean;AberturaSistema:Boolean);
var ArqTEF, aNSU : String;
//    F: TSearchRec;
//    Ret : Integer;
    aControle : Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      if aNSU <> '' then
      begin
         MessageDlg('Transação TEF efetuada. Favor re-imprimir último cupom.'+#13+#10+
                    'NSU(s): '+Copy(aNSU,1,length(aNSU)-1)+#13+#10+
...
//      IncrementaContadorCER;
      ...
      while not Q.Eof do
      begin
        Aux := 'Orig.: '+Trim(Q.FieldByName('numero_mesa_orig').AsString)+' '+
               'Dest.: '+Trim(Q.FieldByName('numero_mesa').AsString)+' '+
               'Prod.: '+copy(Q.FieldByName('descricao').AsString,1,22)+' '+
               'Qtde: '+FormatFloat('0.000',Q.FieldByName('quant').AsFloat)+' '+
               'Valor: '+FormatFloat('0.00',Q.FieldByName('preco_total').AsFloat);
...
procedure TFuncoesPAF.EmiteGerencialSangria(pCaixa, pOperador: String;
  pData: TDateTime; pValor: Double);
begin
  Abre_RelatorioGerencial(gIndiceGerencial);


//fabricio - acrescentar
//fabricio novo pergunta pra imprimir
  if confirmacao('Deseja imprimir a Sangria ? ') then
     DmWorkCom.Imprimir_suprsangr('SANGRIA', Pcaixa,Poperador,FloatToStr(Pvalor),Pdata);

  //fabricio inicio - tirar
{
...
procedure TFuncoesPAF.EmiteGerencialVendaPrazo(pData: TDateTime;
  pValor: Double; pCliente: String);
begin
  Abre_RelatorioGerencial(gIndiceGerencial);
...
procedure TFuncoesPAF.ImprimirFechamentoCaixaNFCe(Operador, Caixa: String;
  Valor: Double);
var aDevice, aDriver, aPort: array[0..255] of char;
    ...
    HPrinter : THandle;
begin
  OpenPrinter(PChar(gImpressora), HPrinter, nil);
  Doc.pDocName    := 'Teste';
  Doc.pOutputFile := '';
  Doc.pDatatype   := 'RAW';
  StartDocPrinter(HPrinter, 1, @Doc);

  s := String('Relatorio de fechamento de caixa') + #13+#10;
  s := s + 'Data do Fechamento: '+FormatDateTime('HH:MM',Now)+#13 + #10;
  s := s + 'Hora da Fechamento: '+FormatDateTime('HH:MM',Now)+#13 + #10;
  s := s + 'Operador Atual: '+Operador+#13 + #10;
  s := s + 'Caixa aberto para: '+Caixa+#13 + #10;
  s := s + 'Valor informado na sangria: '+FormatFloat('0.00',Valor)+#13 + #10;
  s := s + ''+#13+#10;
  s := s + '------------------------------------------------'+#13+#10;
  s := s + ''+#13+#10+#12;
//  s := s + ''+#13+#10+#13+#10+#13+#10+#13+#10+#13+#10+#27+#105;
...
function TFuncoesPAF.RetornaSaldoCaixa(pCaixa: String;
...
var cLinhaArquivo, cCampoArquivo, aNSU, aNomeRede: string;
    cArquivo: TextFile;
    Q : TSQLQuery;
begin
  Result := false;
...
procedure TFuncoesPAF.GravaRegistrodePerdaImpresso(pID,
  pImpresso: Integer);
var Q : TSQLQuery;
begin
  Q := TSQLQuery.Create(nil);
...
procedure TFuncoesPAF.RelatorioAfericao(Codigo: Integer; Bico,
  Combustivel: String; quantiade, EI, EF: Double);
var Texto : String;
begin
  Texto := '#AB:B'+Bico+' EI'+ZeroEsquerda(FormatFloat('0.000',EI),11)+' EF'+ZeroEsquerda(FormatFloat('0.000',EF),11)+' V'+FormatFloat('0.000',quantiade);
...
procedure TFuncoesPAF.RelatorioDescontinuidadeEncerrante(Bomba, Bico,
  Motivo, Justifica, Justifica1, Justifica2, Medidas, Medidas1,
  Medidas2: String; Data, hora: TDateTime; EI, EF: Double);
var Texto : String;
    IDProduto : Integer;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      Texto := '#EF:TA='+ZeroEsquerda(FieldByName('TANQUE_NUMERO').AsString,2)+' '+
                'VOL='+AuxVol+' '+FieldByName('DESCRICAO').AsString;
...
procedure TFuncoesPAF.RelatorioPerdaCombustive(
  CdsTanquePerda: TClientDataSet);
var Texto, Volume : String;
begin
  ...
  begin
    ...
    begin
      ...
      Texto  := '#PE:TQ='+ZeroEsquerda(FieldByName('TANQUE_NUMERO').AsString,2)+' '+
                'VOL='+Volume+' '+FieldByName('DESCRICAO').AsString;
      ImprimeRelatorioGerencial(Texto);
...
procedure TFuncoesPAF.RelatorioPerdaComunicacao(
...
begin
  ...
  begin
    ...
    begin
      ...
      ImprimeRelatorioGerencial('#PC:B'+ZeroEsquerda(FieldByName('Num_Bico').AsString,2)+' EF'+ZeroEsquerda(FormatFloat('0.000',FieldByName('EF').AsFloat),11));
      ImprimeRelatorioGerencial('                                            ');
      // grava o registro como impresso
...
procedure TFuncoesPAF.RelatorioRetornoComunicacao(
  CdsBicoComunica: TClientDataSet);
...
//      ImprimeRelatorioGerencial('#RC:B'+ZeroEsquerda(FieldByName('Num_Bico').AsString,2)+' EF'+ZeroEsquerda(FormatFloat('0.000',EncConcentrador),11));
      ImprimeRelatorioGerencial('#RC:B'+ZeroEsquerda(FieldByName('Num_Bico').AsString,2)+' EF'+ZeroEsquerda(FormatFloat('0.000',FieldByName('EI').AsFloat),11));
      ImprimeRelatorioGerencial('                                            ');
      // grava o registro como impresso
...
function TFuncoesPAF.VerificaRegistro1320(pBico: String;
...
function TFuncoesPAF.RetornaTotalAbastcimentoCupom(
  pIDCupom: Integer): Integer;
var Q : TSQLQuery;
begin
  Result := 0;
  Q := TSQLQuery.Create(nil);
...
procedure TFuncoesPAF.EmiteGerencialVidaLink(Texto, pCOO,
  pFormaPag: String; pValor: double);
var aOK : Boolean;
begin
  try
    aOK := true;
...
//    IncrementaContadorCER;
    ...
    begin
      Aux := 'Numero: '+Trim(pCdsMesas.FieldByName('numero_mesa').AsString)+' '+
             'Data: '+FormatDateTime('dd/mm/yyyy',pCdsMesas.FieldByName('data').AsDateTime)+' '+
             'Hora: '+FormatDateTime('hh:mm:ss',pCdsMesas.FieldByName('hora').AsDateTime);
      ImprimeRelatorioGerencial(aux);
      //
      pCdsMesas.Next;
    end;
...
procedure TFuncoesPAF.EmiteGerencialVendaPrazoNFCe(pData: TDateTime;
  pValor: Double; pCliente: String);
var prn:TCharPrinter;
begin
...
procedure TFuncoesPAF.EmiteGerencialVidaLinkNFCe(Texto, pCOO,
  pFormaPag: String; pValor: double);
var aOK : Boolean;
    prn:TCharPrinter;
begin
...
function TFuncoesPAF.ImprimeTransacaoTEFNFCe(pTipoRel, pVias,
  pControle: Integer; pFormaPag, pCOO: String; pValor: Double): Boolean;
var cLinhaArquivo, aTextoLinha, aux, aArquivo: string;
    cArquivo  : TextFile;
    aLinha, i : Integer;
    auxBol : LongBool;
...
begin
  ...
  begin
    ...
    try
      ...
      AssignPrn(text);
      Rewrite(text);
      s := String('           CUPOM TEF VINCULADO           ') + #13+#10;
    except
      Result := false;
      Exit;
    end;
        ...
        begin
          ...
          begin
            ...
            begin
              Inc(aLinha);
              aTextoLinha := copy(cLinhaArquivo,11,Length(cLinhaArquivo)-10);
              aTextoLinha := RemoveAspas(aTextoLinha)+#13+#10;
              try
                s := s + aTextoLinha + #13 + #10;
              except
                Result := false;
                CloseFile(cArquivo);
                exit;
...
procedure TFuncoesPAF.EmiteGerencialSangriaNFCe(pCaixa, pOperador: String;
  pData: TDateTime; pValor: Double;pMotivoRetirada:String);
var aDevice, aDriver, aPort: array[0..255] of char;
    aMode : THandle;
    Doc:Doc_Info_1;
    iRet:Longword;
...
begin
  ...
  // Envia seu texto
  s := String('Sangria de Caixa') + #13+#10;
  s := s + ''+#13+#10;
  s := s + 'Data da Sangria: '+FormatDateTime('DD/MM/YYYY',Now)+#13 + #10;
  s := s + 'Hora da Sangria: '+FormatDateTime('HH:MM',Now)+#13 + #10;
  s := s + 'Operador Atual: '+pOperador+#13 + #10;
  s := s + 'Valor da sangria: '+FormatFloat('0.00',pValor)+#13 + #10;
  s := s + ''+#13+#10;

  if pMotivoRetirada <> '' then
  begin
    s := s + pMotivoRetirada+#13+#10;
    s := s + #13+#10;
    s := s + #13+#10;
    s := s + #13+#10;
    s := s + #13+#10;
...
procedure TFuncoesPAF.EmiteGerencialSSuprimentoNFCe(pCaixa,
  pOperador: String; pData: TDateTime; pValor: Double);
var aDevice, aDriver, aPort: array[0..255] of char;
    aMode : THandle;
    Doc:Doc_Info_1;
    iRet:Longword;
...
begin
  ...
  Doc.pDatatype   := 'RAW';
  StartDocPrinter(HPrinter, 1, @Doc);
//fabricio novo reduzido as palavras abaixo pra caber na impressao
  s := String('Suprimento') + #13+#10;
  s := s + 'Data: '+FormatDateTime('dd/mm/yy',Now)+#13 + #10;
  s := s + 'Hora: '+FormatDateTime('HH:MM',Now)+#13 + #10;
  s := s + 'Op: '    +pOperador +#13 + #10;
  s := s + 'Valor: '+ FormatFloat('0.00',pValor)+#13 + #10;
  s := s + ''+#13+#10;
  s := s + ''+#13+#10;
  s := s + ''+#13+#10;
  s := s + ''+#13+#10;
...
function TFuncoesPAF.ArqAC1704(pArquivo, pInicio, pFim: String;
  pDatai,pDataf:TDateTime; pinterval: Boolean): Boolean;
var Datai, Dataf : TDateTime;
begin
  // pinterval - true  = por data
  //             false = por redução
...
procedure TFuncoesPAF.CapturaDadosUltimaZBanco(var CRZ, CRO: Integer;
  var VB: Double; NumSerie:String);
var Q : TSQLQuery;
begin
  CRZ := 0;
  CRO := 0;
...
//   Sleep(1000); alterado para ficar mais rapido
   ...
   gIdentifica := FormatDateTime('hhmmss',Time);
   cConteudoArquivo := '';
   cConteudoArquivo := '000-000 = CEL' + #13 + #10+
                       '001-000 = ' + gIdentifica  + #13 + #10 +
                       '150-001 = ' + pDDD + #13 + #10 +
                       '150-002 = ' + pNumTel + #13 + #10 +
                       '210-036 = ' + pCEP + #13 + #10 +
                       '999-999 = 0';
   AssignFile(cArquivo,gPathReqTEF+'IntPos.tmp');
   ReWrite(cArquivo);
