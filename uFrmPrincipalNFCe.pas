...
type
  TFrmPrincipalNFCe = class(TForm)
    ...
    PedidosemContingncia1: TMenuItem;
...
var
  ...
  dadoscaixaid: integer; //fabricio incluir variavel
implementation
...
uses uLibrary, uFrmEmiCupom, uDmWorkCom, uFrmLoginUsuario, uFrmConfiguracoes,
  ...
  uFrmFechaCaixa, uFuncoesPAFECF, uDMClientes, uDmProdutos, uFrmCaixaDiario, uFrmContas; //fabricio incluido uFrmCaixaDiario, uFrmContas;
...
procedure TFrmPrincipalNFCe.ConfigurarImpressora1Click(Sender: TObject);
begin
Winexec('cupom.exe',SW_SHOWNORMAL);
end;

procedure TFrmPrincipalNFCe.ContasaReceber1Click(Sender: TObject);
begin
  try
...
begin
  ...
  begin
    ...
    begin
//fabricio novo
      MessageDlg('A Data do caixa aberto é mais antiga que data atual do sistema. Verfique!', mtWarning, [mbOK], 0);
//fabricio      Abort;
    end;
    // localiza o caixa
//FABRICIO - AQUI TEM Q CONSULTAR O ID

    if DmWorkCom.Dados_Caixa(DataCaixa,true) then
// pegar id do caixa
    DadosCaixaId:= DmWorkCom.CdsCaixaID.AsInteger;
...
var aValor, NomeOperador, MotivoRetirada : String;
    aValorAux : Double;
    DataCaixa : TDateTime;
begin
  // Abre Jornada Fiscal
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            // se estiver marcado para pedir o motivo
            MotivoRetirada := '';
            MotivoRetirada := InputBox('Motivo da retiada','Informe o motivo','Sangria operacional');
            if Trim(MotivoRetirada) = '' then
            begin
              ShowMessage('É necessário informar o motivo da retirada!');
              Abort;
            end;
            if Length(Trim(MotivoRetirada)) < 15 then
            begin
              ShowMessage('O motivo da retirada deve conter pelo menos 15 caracteres!');
              Abort;
            end;
//fabricio novo
          ...
          begin
            ...
            DmWorkCom.CdsCaixaMov.FieldByName('DATA_MOV').AsDateTime   := NOW; //fabricio incluir esta linha
...
//fabricio novo pergunta pra imprimir
          if confirmacao('Deseja imprimir o Suprimento? ') then
             DmWorkCom.PAFECF.EmiteGerencialSSuprimentoNFCe(DmWorkCom.PAFECF.gCaixa,
                                                         DmWorkCom.PAFECF.gOperador,
                                                         now,aValorAux);
        end;
        // grava registro de movimento
//fabricio tirar         if DmWorkCom.Dados_Caixa(SVData,True) then
        if DmWorkCom.Dados_Caixaid(dadoscaixaid,True) then       //fabricio acrescentar esta linha
          lbdadoscaixaid.Caption:= IntToStr(dadoscaixaid);  //fabricio acrescentar esta linha
        begin
          DmWorkCom.Dados_CaixaMov(0);
          DmWorkCom.CdsCaixaMov.Append;
...
//FABRICIO  .asdatetime
         ...
         lbdadoscaixaid.Caption:= DmWorkCom.CdsCaixaID.AsString;
         Grava_Dados(DmWorkCom.CdsCaixaMov);
       end;
       DmWorkCom.PAFECF.AtualizaINI(false);
     end;
...
//fabricio else inicio
              else
              begin
              if confirmacao ('Deseja Imprimir o Fechamento? ') then
                 DmWorkCom.PAFECF.EmiteGerencialSangriaNFCe(DmWorkCom.PAFECF.gCaixa,
                                                            DmWorkCom.PAFECF.gOperador,
                                                            now,aValorTotal,
                                                            'FECHAMENTO DE CAIXA');
