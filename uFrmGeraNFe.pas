...
//fabricio inicio
  ...
  if DmWorkCom.sAmbiente = 0 then
     Label4.Caption:= 'Ambiente: Produção'
  else
     Label4.Caption:= 'Ambiente: Homologação';
...
//           SqlQuery1.SQL.Add('and MODELO = ' + QuotedStr('65'));
        ...
        Edit1.Text:= SqlQuery1NUMERO_DOC.AsString;
         ...
         begin
            ...
            begin
              if MessageDlg('Num.NFE não coincide com a última NFE. Cancelar a Validação da NFE?', mtConfirmation, [mbYes, mbNo], 0) = mryes then
              abort;
            end;
         end;
{
...
//fabricio inicio
  ...
  begin
    if aprocolo = '' then
       ShowMessage('Erro de internet ou na SEFAZ/MS. Protocolo não encontrado, Informe o Fabricio !')
    else
    if arecibo = ''then
       ShowMessage('Erro de internet ou na SEFAZ/MS. Recibo não encontrado, Informe o Fabricio !')
    else
    if aChaveNfe = ''then
       ShowMessage('Erro de internet ou na SEFAZ/MS. Chave não encontrada, Informe o Fabricio !')
    else
    if aStatus = ''then
       ShowMessage('Erro de internet ou na SEFAZ/MS. Status não encontrado, Informe o Fabricio !');
  end;
...
{
    if aprocolo = '' then
       ShowMessage('Falha de internet ou na SEFAZ/MS. Protocolo não encontrado, Informe o Fabricio !')
    else
    if arecibo = ''then
       ShowMessage('Falha de internet ou na SEFAZ/MS. Recibo não encontrado, Informe o Fabricio !')
    else
    if aChaveNfe = ''then
       ShowMessage('Falha de internet ou na SEFAZ/MS. Chave não encontrada, Informe o Fabricio !')
    else
    if aStatus = ''then
       ShowMessage('Falha de internet ou na SEFAZ/MS. Status não encontrado, Informe o Fabricio !');
...
//fabricio inicio - incluido no popmenu1 a opcao altera numero nfe
  ...
  begin
      ...
      if InputQuery('Pesquisa','Informe o número da NFe :',numnfe) then
      begin
...
begin
  ...
  begin
     CarregaDadosXml;
     if InputQuery('Chave NFe','Informe a chave da NFe de referência:',chavedanfe) then
     begin
     achavenfe:= '';
...
//     FrmNfeReferencia.ShowModal;
     ...
     ShowMessage('É necessário validar a Chave NFe');
end;
